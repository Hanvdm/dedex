...
begin
  ...
  if (State = gsNormal) and (Y <= RowHeights[0]) then begin
    Coord := MouseCoord(X, Y);
    CalcDrawInfo(FixedInfo);
    if (CellRect(Coord.X, 0).Right - 5 < X) then begin
      State := gsColSizing;
      Index := Coord.X;
      SizingPos := X;
      SizingOfs := X - CellRect(Coord.X, 0).Right;
    end;
    if (CellRect(Coord.X, 0).Left + 5 > X) then begin
      State := gsColSizing;
      Index := Coord.X - 1;
      SizingPos := X;
...
begin
  if Value <> FEmptyIsNull then begin
    FEmptyIsNull := Value;
...
begin
  ...
  if FLEmptyIsNull then SetEmptyIsNull(ZeroEmpty)
end;
...
//Polaris  if (Value = 0) and ZeroEmpty then FDataLink.Field.Clear
  if (Trim(Text) = EmptyStr) and FEmptyIsNull then FDataLink.Field.Clear
  else if FDataLink.Field.DataType in [ftSmallint, ftInteger, ftWord] then
...
begin
  if VarIsNull(Value) or VarIsEmpty(Value) then FDataLink.Field.Clear
  else FDataLink.Field.Value := CheckValue(Value, False);
  DoChange;
end;

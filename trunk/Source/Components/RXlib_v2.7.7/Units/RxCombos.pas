...
//    for I := 0 to Pred(FNumColors{ColorsInList}) do begin  // Polaris
    for I := 0 to Pred(ColorsInList) do begin  // Polaris
{$IFDEF RX_D3}
      if ((ColorValues[I] = clDefault) and not (coIncludeDefault in Options)) or
         ((ColorValues[I] = clNone) and not (coIncludeNone in Options))
      then
        Continue;

      if (I >= FNumColors) and
         (ColorValues[I] <> clDefault) and
         (ColorValues[I] <> clNone)
      then Continue;

{$ENDIF}
      if (I <= Pred(FColorNames.Count)) and (FColorNames[I] <> '')
      then ColorName := FColorNames[I]
{$IFDEF RX_D3}
...
begin
  if AllColors <> Value then begin
    if not Value then FNumColors := BaseColorsNum
    else FNumColors := ColorsInList-2;
    PopulateList;
    SetColorValue(FColorValue);
  end;
end;
...
begin
  ...
  case Message.NotifyCode of
   CBN_SELENDOK
   {CBN_SELCHANGE}: if FInternalDropDown then begin
     B := FInternalDropDown;
